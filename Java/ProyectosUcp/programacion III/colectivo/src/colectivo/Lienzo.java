...
public class Lienzo extends javax.swing.JPanel implements Runnable {
    ...
    ImageIcon artifi = new ImageIcon(new ImageIcon(getClass().getResource("/img/fuegosArtificiales.gif")).getImage());
    private int x = 0, y = 50, yInicial, numerito = 500, contador = 0, tXM, no, posis;
    private float v, teta, Xmax, tiempo, yMax, areaBajoCurva;
    private boolean rein = false, voltaje = false, m = true, arti = false, acess = true;
    ...
    public boolean isArti() {
        return arti;
    }

    public void setArti(boolean arti) {
        this.arti = arti;
    }
    ...
    public void paint(Graphics g) {     //dibujar
        ...
        if (isArti()) {
            ...
            for (int i = 0; i < 100; i++) {
                g.drawImage(artifi.getImage(), getWidth() / 2, getHeight() / 2, 500, 513, null);
            }
    ...
    public void run() {
        try {
            while (m) {
                ...
                while (y >= 50) {
                    repaint();
                    y = (int) (((tan(teta) * x) - ((9.8 * (pow(x, 2))) / (2 * (pow(v, 2)) * (pow(Math.cos(teta), 2))))));
                    y += yInicial + 50;
                    Thread.sleep(2);
                    x += 1;
                    posx.add(x);
